...
export const DEVICE_REMOTE_NOTIFICATIONS_REGISTERED_EVENT =
  "remoteNotificationsRegistered";
export const DEVICE_REMOTE_NOTIFICATIONS_REGISTRATION_FAILED_EVENT =
  "remoteNotificationsRegistrationFailed";
export const DEVICE_PUSH_KIT_REGISTERED_EVENT = "pushKitRegistered";
export const DEVICE_NOTIFICATION_RECEIVED_FOREGROUND_EVENT =
  "notificationReceivedForeground";
export const DEVICE_NOTIFICATION_OPENED_EVENT = "notificationOpened";
export const DEVICE_PUSH_KIT_NOTIFICATION_RECEIVED_EVENT =
...
export default class NotificationsIOS {
  ...
  static addEventListener(type: string, handler: Function) {
    if (_exportedEvents.indexOf(type) !== -1) {
      ...
      if (type === DEVICE_REMOTE_NOTIFICATIONS_REGISTERED_EVENT) {
        ...
        type === DEVICE_REMOTE_NOTIFICATIONS_REGISTRATION_FAILED_EVENT
      ) {
        listener = DeviceEventEmitter.addListener(
          DEVICE_REMOTE_NOTIFICATIONS_REGISTRATION_FAILED_EVENT,
          error => handler(error)
        ...
        listener = DeviceEventEmitter.addListener(
          type,
          ({ payload, identifier }) =>
            handler(new IOSNotification(payload), presentingOptions => {
              NativeRNNotifications.finishPresentingNotification(
                identifier,
                presentingOptions
              );
            })
        ...
        listener = DeviceEventEmitter.addListener(
          type,
          ({ payload, identifier, action }) =>
            handler(
              new IOSNotification(payload),
              () => {
        ...
        listener = DeviceEventEmitter.addListener(type, payload =>
          handler(new IOSNotification(payload))
        );
      }
  ...
  static requestPermissions(categories: Array<NotificationCategory>) {
    ...
    NativeRNNotifications.requestPermissionsWithCategories(
  ...
  static async getLastAction() {
    const action = await NativeRNNotifications.getLastAction();
